        ...
        <div class="content select-panel">
          <el-tabs v-model="activated" @tab-click="handleTabClick">
            <el-tab-pane label="全部" name="0">
              <el-table
                :data="feedback_list.slice(page_start,page_end)"
                style="width: 100%"
              ...
              >
                ...
                <el-table-column label="反馈时间" width="220">
                  <template slot-scope="scope">
                    <p>{{ scope.row.feedback_time | getFullTime}}</p>
                  </template>
                </el-table-column>
            ...
            <el-tab-pane label="已读" name="1">
              <el-table
                :data="read_feedback_list.slice(page_start,page_end)"
                style="width: 100%"
                highlight-current-row
                @row-click="getDetail"
              >
            ...
            <el-tab-pane label="未读" name="2">
              <el-table
                :data="unread_feedback_list.slice(page_start,page_end)"
                style="width: 100%"
                highlight-current-row
                @row-click="getDetail"
              >
...
export default {
  ...
  methods: {
    handlePageChange: function(val) {
      ...
      switch (this.activated) {
        case "0":
          ...
          this.page_end = Math.min(
            this.page_start + this.page_size,
            this.feedback_list.length
          );
          break;
        case "1":
          ...
          this.page_end = Math.min(
            this.page_start + this.page_size,
            this.read_feedback_list.length
          );
          break;
        case "2":
          this.total_len = this.unread_feedback_list.length;
          this.page_start = Math.min(
            this.page_start + this.page_size,
            this.unread_feedback_list.length
          );
          break;
        default:
          break;
    ...
    getDetail(row, event, column) {
      let feedback_id = parseInt(
        event.currentTarget.getElementsByClassName("feedback-id")[0].innerText
      );
